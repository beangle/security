...
case class RoleAuthority(resourceName: String, scope: Scopes.Scope, roles: Set[String]) {
  ...
  def matches(authorityString: String): Boolean = {
    val authorities = Strings.split(authorityString, ",")
    authorities.exists(roles.contains)
  }

  def matches(authorities: collection.Set[String]): Boolean = {
    roles.exists(authorities.contains)
  }
}
