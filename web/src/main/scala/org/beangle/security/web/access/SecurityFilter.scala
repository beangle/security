...
import org.beangle.commons.web.filter.{ MatchedCompositeFilter, VirtualFilterChain }
...
class SecurityFilter(filters: List[Filter], val registry: SessionRegistry, val entryPoint: EntryPoint, val accessDeniedHandler: AccessDeniedHandler)
  ...
  override def doFilter(req: ServletRequest, res: ServletResponse, chain: FilterChain): Unit = {
    try {
      ...
      if (!breakChain) new VirtualFilterChain(chain, getFilters(req).iterator).doFilter(req, res)
    } catch {
      case bse: SecurityException => handleException(req, res, bse)
  ...
  protected def sendStartAuthentication(request: ServletRequest, response: ServletResponse, reason: AuthenticationException): Unit = {
    SecurityContext.session = null
